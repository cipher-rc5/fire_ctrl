...
- Start postgresql@17 and redis as background services
...
Services (postgresql@17, redis) are registered as launchd agents by
...
- Rust is pinned to 1.95 via `Cargo.toml` and `rust-toolchain.toml`.
- If using rustup, run `rustup override set 1.95.0` in this repo.
